...
VIEWS.subject = function (st) {
  ...
  if (!u || !S) return bail();
  const hasG = !!S.groups;
  const tab = st.tab === 'freq' ? 'freq' : 'unit';

  let g = null;
  if (hasG) {
    g = st.g || (tab === 'freq' ? 'all' : S.groups[0]);
  ...
  const cur = { v: 'subject', uid: u.id, sid: S.id, tab, g, u: uId };
  ...
  const unitNow = units.find(x => x.id === uId);
  out.push(fabBtn(() => openProbForm({
    uid: u.id, sid: S.id,
    groups: hasG && g !== 'all' ? [g] : (unitNow && unitNow.group ? [unitNow.group] : []),
